...
export function canAffordUpgrade(upgradeId: string): boolean {
    ...
    return Object.entries(upgrade.currentPrice).every(([resourceId, cost]) => 
...
export function checkRequirements(requirements: Record<string, number>, upgradeId: string): boolean {
...
export function getAvailableUpgrades() {
    return getVisibleUpgrades()
        .filter(id => canAffordUpgrade(id));
}
